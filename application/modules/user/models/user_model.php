...
class User_model extends CI_Model{
    ...
    public function data_edit($id){
        ...
        //$data['backlink'] = $this->get_backlink($data['ID']); //print_r ($data['backlink']); exit;        
        return $data;
    }
    function save_user_link_lists()
    {
        $uid = decode_me($this->input->post('tokenid'));
        $db_data = array('link_lists' => $this->input->post('link_lists') );
        $this->db->update('mod_user_backlink',  $db_data, array('user_id' => $uid ));
        $this->lib_url->reload('Cập nhật thành công...');
    }
    function get_backlink($uid)
    {
       
        $links = array();
        $links += $this->db->get_where('mod_user_backlink', array('user_id' => $uid) ) -> row_array();
        if(!count($links))
        {
            $this->db->insert('mod_user_backlink', array('user_id' => $uid ));
        }
    	return $links;
     }
    ...
    public function do_forgot(&$data){
        ...
        if($this->check_exist_email($email_to)){
			...
			$data['error_messenger'] = "<p class='red_note'>Có lổi sãy ra khi gửi email, vui lòng thử lại.</p>";	
        }
        else 
        {
