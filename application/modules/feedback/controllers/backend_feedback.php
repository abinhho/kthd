...
class Backend_feedback extends MX_Controller {
	
	var $_GET_id;
	public $title = "Quản lý các ý kiến phản hồi";
	public function __construct(){
       	parent::__construct();
      	$this->load->Model('Backend_feedback_model');
	...
	public function home(){
		...
		$data = array();
		$data = $this->Backend_feedback_model->home();
	...
	public function show_item(){
		...
		if($submit)
		{
			...
			{
				$this->Backend_feedback_model->del_multi();
			}		
		}
		$data = $this->Backend_feedback_model->show_item();
		$this->load->view("backend_show_item",$data);
	}
	
	public function view_item(){
		
		$data = $this->Backend_feedback_model->view_item($this->_GET_id);
		$this->template->write('title',$this->title, true);
	...
	public function del(){
		...
		$this->Backend_feedback_model -> del($this->_GET_id);
