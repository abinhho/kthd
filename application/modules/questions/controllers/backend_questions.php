...
class Backend_questions extends MX_Controller {
	
	var $_GET_id;
	var $module_alias = "questions";
	public $title = "Quản lý bài viết";
	public function __construct(){
       	parent::__construct();
      	$this->load->Model('Backend_questions_model');
	...
	public function show_item(){
		...
		if($submit)
		{
			...
			{
				$this->Backend_questions_model->del_multi();
			}		
		}
				
		$data = $this->Backend_questions_model->show_item();
	...
	public function del(){
		...
		$this->Backend_questions_model -> del($this->_GET_id);
	...
	public function edit(){
		...
		$data_model = ($this->_GET_id != "") ? $this->Backend_questions_model->data_edit($this->_GET_id) : array() ;
		...
		if($submit)
		{
			...
			if($this->form_validation->run())
			{
				$this->Backend_questions_model->do_edit($data, $this->_GET_id);
			}
	...
	public function copy(){
		...
		$this->Backend_questions_model->copy($this->_GET_id);
		$this->lib_url->redirect_to_back();
	}
	
    public function system_clean()
    {
        echo modules::run('ext_system_clean','questions');
    }
    ...
    {
        echo modules::run('com_blocks/backend_com_blocks/blocks_each_module', "questions");
